...
// Copyright (c) 2011-2013 DieselCoin Developers.
...
uint256 hashGenesisBlock("0x7f8a8ceee4a05f5ee70edb05a64103f8a0cf0c1bddf6e1c3eb4fd7438b6ef92b");
...
const string strMessageMagic = "DieselCoin Signed Message:\n";
...
{
    int64 nSubsidy = 300 * COIN; // 300 coins per block
	
	if (nHeight == 1)
		nSubsidy = 6307200 * COIN;
	
	nSubsidy >>= (nHeight / 1051200);	// subsidy halves every 4 years

    return nSubsidy + nFees;
}

static int64 nTargetTimespan =  30 * 2 * 60; // 30 blocks
static int64 nTargetSpacing = 2 * 60; // 120 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
	...
	if (pindexLast->nHeight+1 < 370) {
		nTargetTimespan =  30 * 120 * 60;
		nTargetSpacing = 120 * 60;
		nInterval = nTargetTimespan / nTargetSpacing; }
	else {
		nTargetTimespan =  30 * 2 * 60; // 30 blocks
    ...
    // DieselCoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal DieselCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DieselCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
// Genesis block
        const char* pszTimestamp = "'Gilligan's Island' Professor, actor Russell Johnson, dead at 89";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
		...
		assert(block.hashMerkleRoot == uint256("0xe8ab5ed41c45a9c938bedf880366fe6d0a89ffbb36df3ff59d53b1d29098130a"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xcf, 0xa4, 0xc7 }; 
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // DieselCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
